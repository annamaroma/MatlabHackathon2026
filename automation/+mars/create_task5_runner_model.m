...
source_solar_block = find_top_level_block(task3_model, {'Solar Array'});
source_mppt_block = find_top_level_block(task3_model, {'MPPT'});
if ~exist_block(solar_block)
    add_block(source_solar_block, solar_block, 'Position', [225, 215, 365, 295]);
end
if ~exist_block(mppt_block)
    add_block(source_mppt_block, mppt_block, 'Position', [500, 70, 680, 185]);
end
...
source_setpoint_block = find_top_level_block(task4_model, {'Voltage Set Point', sprintf('Voltage \nSet Point')});
source_sum_block = find_top_level_block(task4_model, {'Sum'});
source_integrator_block = find_top_level_block(task4_model, {'Discrete-Time Integrator', sprintf('Discrete-Time\nIntegrator')});
if ~exist_block(setpoint_block)
    add_block(source_setpoint_block, setpoint_block, 'Position', [705, 205, 735, 235]);
end
if ~exist_block(sum_block)
    add_block(source_sum_block, sum_block, 'Position', [760, 208, 780, 228]);
end
if ~exist_block(integrator_block)
    add_block(source_integrator_block, integrator_block, 'Position', [815, 202, 850, 238]);
end
...
function block_path = find_top_level_block(model_name, candidate_names)
hits = {};
