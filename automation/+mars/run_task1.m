...
if isempty(passing_rows)
    ...
    best_result.selection_rule = 'minimum solar_kW among passing runs';
    return;
end
...
% Rebuild grids in original simulation order so heatmap cells line up with ndgrid indexing.
plot_table = sortrows(summary_table, 'run_index', 'ascend');
pass_matrix = reshape(plot_table.pass, numel(solar_values), numel(soc_values));
completion_matrix = reshape(plot_table.mission_fraction, numel(solar_values), numel(soc_values));
final_soc_matrix = reshape(plot_table.finalSOC_pct, numel(solar_values), numel(soc_values));
...
fig_handle = figure('Visible', 'off', 'Color', 'k', 'Tag', 'task1_pass_heatmap');
imagesc(soc_values, solar_values, double(pass_matrix));
set(gca, 'YDir', 'normal');
colormap(fig_handle, [0.80, 0.27, 0.27; 0.22, 0.66, 0.33]);
caxis([0, 1]);
color_handle = colorbar('Ticks', [0, 1], 'TickLabels', {'Fail', 'Pass'});
xlabel('Initial SOC (%)');
...
hold off;
local_style_axes_dark(gca);
local_style_colorbar(color_handle);
end

function fig_handle = local_plot_completion_heatmap(completion_matrix, solar_values, soc_values, best_result)
fig_handle = figure('Visible', 'off', 'Color', 'k', 'Tag', 'task1_completion_heatmap');
...
fig_handle = figure('Visible', 'off', 'Color', 'k', 'Tag', 'task1_final_soc_heatmap');
...
fig_handle = figure('Visible', 'off', 'Color', 'k', 'Tag', 'task1_best_trace');
...
if ~best_result.found
    ...
    title(sprintf('Minimum Passing Solar Run: %g kW, %g%% initial SOC', best_result.solar_kW, best_result.initSOC_pct));
    local_style_axes_dark(gca);

    nexttile;
    local_plot_signal(best_result.signals.battPower_kW, stop_time, 'Battery Power (kW)', []);
    yline(0, '--', 'Zero Power', 'Color', [0.70, 0.70, 0.70]);
    local_style_axes_dark(gca);
...
if plotted && ~isempty(band)
    hold on;
    yline(band(1), '--', 'Min', 'Color', [0.65, 0.65, 0.65]);
    yline(band(2), '--', 'Max', 'Color', [0.65, 0.65, 0.65]);
    hold off;
end
...
plot(best_result.initSOC_pct, best_result.solar_kW, 'wp', ...
    'MarkerSize', 12, ...
    'MarkerFaceColor', [1, 0.90, 0.1], ...
    'DisplayName', 'Minimum passing solar');
...
function local_style_axes_dark(ax_handle)
if nargin < 1 || isempty(ax_handle) || ~ishandle(ax_handle)
    return;
end
...
set(ax_handle, ...
    ...
    'GridAlpha', 0.35, ...
    'MinorGridColor', [0.40, 0.40, 0.40], ...
    'MinorGridAlpha', 0.25, ...
    'Layer', 'top');

set(get(ax_handle, 'Title'), 'Color', [0.95, 0.95, 0.95]);
set(get(ax_handle, 'XLabel'), 'Color', [0.95, 0.95, 0.95]);
set(get(ax_handle, 'YLabel'), 'Color', [0.95, 0.95, 0.95]);
end

function local_style_colorbar(color_handle)
if nargin < 1 || isempty(color_handle) || ~ishandle(color_handle)
    return;
end

color_handle.Color = [0.95, 0.95, 0.95];
if isprop(color_handle, 'Label') && ~isempty(color_handle.Label)
    color_handle.Label.Color = [0.95, 0.95, 0.95];
end
end
...
if isstruct(best_result) && isfield(best_result, 'found') && best_result.found
    fprintf('[task1] Selected representative run at the minimum passing solar rating: solar=%g kW, initSOC=%g %%, finalSOC=%.2f %%.\n', ...
